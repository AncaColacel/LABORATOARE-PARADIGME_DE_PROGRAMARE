...
mergeSort :: [Int] -> [Int]
mergeSort lst = 
    let computeHalf f l = f (quot (length l) 2) l                   -- spre deosebire de nume variabila = expresie sau numar
                                                                    -- la functie avem nume functie argumente = corp
        fstHalf l = computeHalf take l
        sndHalf l = computeHalf drop l
    in
        case lst of
            [] -> []
            [_] -> lst                                               -- daca lista are un singur element intorc direct lista sortata
            _ -> merge (mergeSort (fstHalf lst)) (mergeSort (sndHalf lst))

mergeSort2 :: [Int] -> [Int]
mergeSort2 lst =  case lst of
            [] -> []
            [x] -> lst                                                 -- alt mod de a scrie ce am zis mai sus
            _ -> merge (mergeSort (fstHalf lst)) (mergeSort (sndHalf lst))
            where 
