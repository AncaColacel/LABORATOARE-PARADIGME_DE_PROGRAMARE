...
myReverse([], []).
myReverse([H|T], REV_LIST) :- myReverse(T, Rev_List) , myConcat(Rev_List, [H], REV_LIST).

%% se apeleaza functia de reverse in mod recursiv si mereu se da ca parametru restul de lista ramasa
%% apoi se foloseste myConcat pentru a concatena rezultatul partial cu headul partial si a forma rev-ul final
